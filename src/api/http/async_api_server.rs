...
pub fn register(lua: &Lua) -> Result<Table> {
    ...
    let lua = Arc::new(lua.clone()); // Arc<Lua> verwenden
    ...
    let start_api_server = {
        ...
        let start_api_server =
            lua.create_function(move |_, (port, handlers_table): (u16, Option<Table>)| {
                let lua_clone = Arc::clone(&lua); // nur clone erzeugen, nicht bewegen
                let lua = Arc::clone(&lua_clone); // lokale clone für diese Closure

                let (shutdown_tx, shutdown_rx) = oneshot::channel();
                server_controls.lock().unwrap().insert(port, shutdown_tx);

                // Sammle alle Lua-Routen
                ...
                if let Some(table) = handlers_table {
                    for pair in table.pairs::<String, LuaValue>() {
                        ...
                        let func: Function = match value {
                            ...
                            LuaValue::String(s) => {
                                let s_val = s;
                                let borrowed = s_val.to_str()?; // BorrowedStr
                                let src: &str = borrowed.as_ref(); // &str gültig solange borrowed lebt
